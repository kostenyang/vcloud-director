<#
.SYNOPSIS
  Phase 2 V3 (FULLY STANDALONE) - Switch VM NICs from each source network
...
.DESCRIPTION
  ...
  For each entry in portGroup.sources[]:
    1. Look up the source + dest Org VDC Networks in VCD (the dest must
       already exist - run Step12-Import-V3.ps1 first).
...
Write-Host "=== Phase 2 V3 (standalone, optimized): switch VM NICs to *-new ===" -ForegroundColor Magenta
...
foreach ($src in $sources) {
    ...
    try {
        ...
        if (-not $destNet) { throw "Destination Org VDC Network '$destName' not found in VDC. Run Step12-Import-V3.ps1 first." }
...
$summary = [ordered]@{
    ...
    createdBy      = 'Step3-Switch-V3.ps1 (standalone)'
    config         = $ConfigPath
